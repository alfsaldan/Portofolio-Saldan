...
/* ═══════════════════════════════════════════════════════════════════
   3D TILT PROFILE CARD — matches reference image 1 proportions
═══════════════════════════════════════════════════════════════════ */
const ProfileCard = ({ isMobile, profileImage }) => {
  ...
  // Larger card matching reference — 380×530 desktop, 280×390 mobile
  ...
  const onEnter = () => { hovering.current = true; setIsHovered(true); setGlare(0); };
  const onLeave = () => {
    hovering.current = false; setIsHovered(false); setGlare(1);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* LAYER 4: Glass sweep idle */}
        <div style={{
          position: "absolute", inset: 0, zIndex: 5, pointerEvents: "none", borderRadius: 20,
          background: `linear-gradient(135deg,
            transparent 0%, transparent 28%,
            rgba(255,255,255,0.04) 42%, rgba(255,255,255,0.14) 50%,
            rgba(255,255,255,0.04) 58%, transparent 72%, transparent 100%)`,
          backgroundSize: "200% 200%",
          animation: isHovered ? "none" : "glassSweep 3.8s ease-in-out infinite",
          ...
          animation: isHovered ? "none" : "glassSweep 8s ease-in-out infinite",
          opacity: glare, transition: "opacity 0.4s",
        }} />
...
export default function Portfolio() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const sections = ["beranda", "tentang", "sertifikat", "proyek", "kontak"];
      let current = "beranda";
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @keyframes glassSweep{0%{background-position:150% 150%}50%{background-position:-50% -50%}100%{background-position:150% 150%}}
        @keyframes glassSweep{0%{background-position:200% 200%}35%{background-position:-100% -100%}100%{background-position:-100% -100%}}
        ...
        .project-card-wide{border-radius:16px;overflow:hidden;position:relative;cursor:pointer;transition:transform .3s}
        .project-card-wide:hover{transform:scale(1.01)}
        .project-card-small{border-radius:16px;overflow:hidden;background:#111;border:1px solid rgba(255,255,255,.06);cursor:pointer;transition:transform .3s}
        .project-card-small:hover{transform:translateY(-5px)}
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <nav style={{
          ...
          {!isMobile && (
            <div style={{ display: "flex", gap: 38, alignItems: "center" }}>
              {["Beranda", "Tentang", "Sertifikat", "Proyek", "Kontak"].map((item) => (
                <a
                  key={item}
        ...
        {/* ── TECH STACK ── */}
        <section style={{ padding: isMobile ? "60px 0" : "80px 0", overflow: "hidden" }}>
        ...
        {/* ── SERTIFIKAT ── */}
        <section id="sertifikat" style={{ padding: isMobile ? "60px 24px" : "80px 64px", background: "rgba(255,255,255,0.01)" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div className="reveal" style={{ textAlign: "center", marginBottom: isMobile ? 40 : 56 }}>
              <h2 style={{ fontFamily: "'DM Sans',sans-serif", fontWeight: 800, fontSize: isMobile ? 32 : 50, color: "#fff", letterSpacing: -0.5 }}>
              ...
              {certificates.length > 0 ? certificates.map((cert, i) => (
                <a key={i} href={cert.url} target="_blank" rel="noreferrer" className="cert-card">
                  <div className="cert-preview">
                    {/* Menggunakan iframe agar file PDF langsung ter-render sebagai preview */}
                    <iframe src={`${cert.url}#toolbar=0&navpanes=0&scrollbar=0&view=FitH`} title={cert.title} />
        ...
        {/* ── PROJECTS ── */}
        <section id="proyek" style={{ padding: isMobile ? "60px 24px" : "80px 64px" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div className="reveal" style={{ textAlign: "center", marginBottom: isMobile ? 40 : 56 }}>
              <h2 style={{ fontFamily: "'DM Sans',sans-serif", fontWeight: 800, fontSize: isMobile ? 32 : 50, color: "#fff", letterSpacing: -0.5 }}>
            ...
            <div style={{
              ...
              {/* Wide card */}
              <div className="project-card-wide" style={{ background: "#111", minHeight: isMobile ? 240 : 420, position: "relative" }}>
                <div style={{
                  width: "100%", height: "100%", minHeight: isMobile ? 240 : 420,
                  background: "linear-gradient(135deg, #f0f0e8 0%, #e8e0d0 100%)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 72, color: "rgba(0,0,0,0.08)",
                }}>🍽</div>
                <div style={{
                  position: "absolute", bottom: 0, left: 0, right: 0,
                  padding: "24px 28px",
                  ...
                  <h3 style={{ color: "#fff", fontFamily: "'DM Sans',sans-serif", fontWeight: 700, fontSize: isMobile ? 20 : 28, marginBottom: 6, letterSpacing: -0.5 }}>Web Self-Service Cafe</h3>
                  <p style={{ color: "rgba(255,255,255,.55)", fontSize: 13, marginBottom: 14, fontWeight: 300 }}>Aplikasi berbasis web, dengan fitur pesan sendiri.</p>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
                    {["html", "css", "js"].map(t => (
                      <span key={t} style={{ padding: "3px 11px", borderRadius: 20, background: "rgba(0,200,150,.15)", border: "1px solid rgba(0,200,150,.35)", color: "#00c896", fontSize: 10, fontFamily: "'Space Mono',monospace" }}>{t}</span>
                    ))}
                  </div>
                  <a href="#" style={{ color: "#00c896", fontSize: 13, textDecoration: "none", fontFamily: "'Space Mono',monospace" }}>Lihat ↗</a>
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  { title: "Dashboard Analytics", desc: "Real-time analytics dashboard dengan visualisasi data.", bg: "#1a1a24", accent: "#6c63ff" },
                  { title: "Brand Identity Pack", desc: "Complete visual identity system untuk startup.", bg: "#1a2424", accent: "#06b6d4" },
                ].map((p) => (
                  <div key={p.title} className="project-card-small" style={{ flex: 1, minHeight: isMobile ? 160 : 196, position: "relative", background: p.bg }}>
                    <div style={{ padding: isMobile ? "20px" : "26px", height: "100%", display: "flex", flexDirection: "column" }}>
                      <div style={{ width: 32, height: 3, background: p.accent, borderRadius: 2, marginBottom: 12 }} />
                      <h3 style={{ color: "#fff", fontFamily: "'DM Sans',sans-serif", fontWeight: 700, fontSize: isMobile ? 16 : 20, marginBottom: 8, letterSpacing: -0.3 }}>{p.title}</h3>
                      <p style={{ color: "rgba(255,255,255,.35)", fontSize: 13, flex: 1, fontWeight: 300, lineHeight: 1.6 }}>{p.desc}</p>
                    </div>
                    <div style={{
                      position: "absolute", inset: 0, borderRadius: 16,
                      background: `radial-gradient(ellipse at 75% 50%, ${p.accent}22, transparent)`,
                      pointerEvents: "none",
                    }} />
                  </div>
                ))}
        ...
        <section id="kontak" style={{ padding: isMobile ? "60px 24px" : "100px 64px" }}>
          <div style={{ maxWidth: 680, margin: "0 auto", textAlign: "center" }}>
            <div className="reveal">
              <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, letterSpacing: 3, textTransform: "uppercase", color: "rgba(255,255,255,.25)", marginBottom: 14, display: "flex", justifyContent: "center" }}>04 · Kontak</div>
        ...
        <footer style={{
          ...
          <div style={{ fontFamily: "'DM Sans',sans-serif", fontWeight: 800, fontSize: isMobile ? 17 : 22, color: "rgba(255,255,255,.25)" }}>
            Alfi<span style={{ color: "#00c896" }}>.</span>
          </div>
          <div style={{ fontFamily: "'Space Mono',monospace", fontSize: isMobile ? 9 : 10, color: "rgba(255,255,255,.15)" }}>© 2024 · Crafted with ♥</div>
        </footer>
      </div>
